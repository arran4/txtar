...
func (c *SkillInspect) Usage() {
	err := executeUsage(os.Stderr, "skill-inspect_usage.txt", UsageDataSkillInspect{c, false})
	if err != nil {
...
func (c *SkillInspect) UsageRecursive() {
	err := executeUsage(os.Stderr, "skill-inspect_usage.txt", UsageDataSkillInspect{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func (c *SkillInspect) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("skill-inspect failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewSkillInspect() *SkillInspect {
	set := flag.NewFlagSet("skill-inspect", flag.ContinueOnError)
	v := &SkillInspect{
		RootCmd:     c,
		Flags:       set,
