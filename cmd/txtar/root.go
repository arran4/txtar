...
func (c *RootCmd) Usage() {
	err := executeUsage(os.Stderr, "txtar_usage.txt", UsageDataRootCmd{c, false})
	if err != nil {
...
func (c *RootCmd) UsageRecursive() {
	err := executeUsage(os.Stderr, "txtar_usage.txt", UsageDataRootCmd{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewAdd() })
		c.Commands["add"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewAppend() })
		c.Commands["append"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewCat() })
		c.Commands["cat"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewComment() })
		c.Commands["comment"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewCreate() })
		c.Commands["create"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewDelete() })
		c.Commands["delete"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewDescription() })
		c.Commands["description"] = subCmd

	}

	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewExtract() })
		c.Commands["extract"] = subCmd
